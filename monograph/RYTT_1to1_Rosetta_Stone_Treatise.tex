...
\rosettapanel{The Bounded Left-Inverse Round-Trip Identity ($\mathcal{D}(\mathcal{C}(c)) \equiv c$)}{%
Let $\mathcal{A}$ be the complete alphabet and $\mathcal{S}^*$ be the free monoid of text strings. What is machine-verified is narrower than the panel's earlier title claimed: the RYTT encoding map $\mathcal{C}$ and decoding map $\mathcal{D}$ satisfy left-invertibility per chord constructor $c$, on the bounded domain tested in \texttt{proofs/RYTT\_standalone.lean} -- \textbf{[thm]}. This is not a proof of bijectivity (which needs injectivity plus surjectivity onto the full valid range) or of an involution ($f(f(x))=x$, a different statement) over the whole string monoid $\mathcal{S}^*$:
$$\forall c \in \{\text{chord constructors}\},\quad \mathcal{D}(\mathcal{C}(c)) = c$$
}{%
$\forall c,\quad \mathcal{D}(\mathcal{C}(c)) \equiv c$ (bounded domain, zero \texttt{sorry} in \texttt{RYTT\_standalone.lean}).\par
$\mathbf{\Sigma}^\uparrow_L \mathbf{\Sigma}_e \mathbf{\Sigma}_a \mathbf{\Sigma}_n~4 \vdash \mathtt{pua\_decode(pua\_encode~c)} = c$ -- \textbf{[thm]}, verified.\par
Everything under \texttt{RYTT.lean} and \texttt{RYTTProofs/} does \textbf{not compile} -- do not cite it as established.
}
...
\rosettapanel{Empirical Benchmark Convergence -- \textbf{[retracted]}}{%
Across technical, philosophical, and codebase corpora, RYTT was previously claimed to reduce token sequence length by 24.8\%--31.2\% compared to raw character streams. Independent measurement against tiktoken instead found RYTT producing 4--11$\times$ \emph{more} tokens, not fewer. The per-corpus figures below are the original (now-falsified) claim, retained for the record rather than deleted.
}{%
