...
fn replay_all_conformance_vectors() {
    ...
    let vectors: Value = serde_json::from_str(include_str!("../../../conformance/vectors.json"))
    ...
    for vector in vectors["vectors"].as_array().expect("vectors array") {
        ...
        let envelope =
            encode(&spec, source).unwrap_or_else(|error| panic!("{id}: encode failed: {error}"));
        assert_eq!(
            envelope.encoded_display, expected_display,
            "{id}: display mismatch"
        );
        assert_eq!(
            envelope.token_trace.len(),
            expected_count,
            "{id}: token count mismatch"
        );
        assert_eq!(
            decode(&spec, &envelope).unwrap(),
            source,
            "{id}: decode mismatch"
        );
    }
