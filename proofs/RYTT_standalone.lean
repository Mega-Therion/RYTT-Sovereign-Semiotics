...
constructor, on bounded domains (`id < 32` for primitives, `id < 100` for
...
The bounds are load-bearing, not decoration: `0xE000 + 32 = 0xE020` is the first
compound codepoint, so at `id = 32` a primitive encoding collides with the
compound region. Verified by widening `id < 32` to `id < 33`, which breaks the
...
def pua_encode (c : RYTTChord) : Nat :=
  ...
  | RYTTChord.compound id SemioticPlane.Ground _   => 0xE020 + id
  | RYTTChord.compound id SemioticPlane.Elevated _ => 0xE820 + id

def pua_decode (n : Nat) (len : Nat := 1) : RYTTChord :=
  if n >= 0xE820 then
    RYTTChord.compound (n - 0xE820) SemioticPlane.Elevated len
  else if n >= 0xE800 then
    RYTTChord.primitive (n - 0xE800) SemioticPlane.Elevated
  else if n >= 0xE020 then
    RYTTChord.compound (n - 0xE020) SemioticPlane.Ground len
  else if n >= 0xE000 then
    RYTTChord.primitive (n - 0xE000) SemioticPlane.Ground
  else
    RYTTChord.primitive 0 SemioticPlane.Ground

/-- Round-trip (left inverse) for Ground primitives, on the domain `id < 32`.
    ...
    `0xE000 + 32 = 0xE020` is the first compound codepoint, so at `id = 32` the
    encoding collides with the compound region and the theorem is false.
    Verified by widening `id < 32` to `id < 33`, which breaks the proof. -/
theorem rytt_ground_primitive_left_inverse (id : Nat) (h : id < 32) :
  ...
  have h1 : ¬(0xE000 + id >= 0xE820) := by omega
  have h2 : ¬(0xE000 + id >= 0xE800) := by omega
  have h3 : ¬(0xE000 + id >= 0xE020) := by omega
  have h4 : 0xE000 + id >= 0xE000 := by omega
  simp [h1, h2, h3, h4]

/-- Round-trip (left inverse) for Elevated primitives, on the domain `id < 32`.
    Left inverse only -- see the Ground primitive docstring. Bound is load-bearing:
    `0xE800 + 32 = 0xE820` is the first Elevated compound codepoint. -/
theorem rytt_elevated_primitive_left_inverse (id : Nat) (h : id < 32) :
    pua_decode (pua_encode (RYTTChord.primitive id SemioticPlane.Elevated)) = RYTTChord.primitive id SemioticPlane.Elevated := by
  dsimp [pua_encode, pua_decode]
  have h1 : ¬(0xE800 + id >= 0xE820) := by omega
...
theorem rytt_ground_compound_left_inverse (id : Nat) (len : Nat) (h : id < 100) :
  ...
  dsimp [pua_encode, pua_decode]
  have h1 : ¬(0xE020 + id >= 0xE820) := by omega
  have h2 : ¬(0xE020 + id >= 0xE800) := by omega
  have h3 : 0xE020 + id >= 0xE020 := by omega
  simp [h1, h2, h3]
...
theorem rytt_elevated_compound_left_inverse (id : Nat) (len : Nat) (h : id < 100) :
  ...
  dsimp [pua_encode, pua_decode]
  have h1 : 0xE820 + id >= 0xE820 := by omega
  simp [h1]
